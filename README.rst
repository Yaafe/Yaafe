...
The consequence is the error ``ERROR: cannot load yaafe packages:  No module named yaafelib``. The simplest way to solve this problem is to add the following line in ``/usr/local/bin/yaafe`` after ``from optparse import OptionParser`` :
