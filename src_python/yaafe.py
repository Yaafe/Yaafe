...
def listFeatures():
    features = [feat.__name__ for feat
                in yaafe.AudioFeatureFactory.get_all_features()
                if not feat.TRANSFORM]
    transforms = [feat.__name__ for feat
                  in yaafe.AudioFeatureFactory.get_all_features()
                  if feat.TRANSFORM]
    if len(features) == 0:
        print ('No features available ! Please check that YAAFE_PATH env var'
               ' is set correctly.')
        return
    features.sort()
    transforms.sort()
...
def main():
    ...
    parser.add_option('-r', '--rate', dest='sample_rate', type='int',
                      default=None,
                      help='working samplerate in Hz. If not set, use input '
                           'file sample rate. ')
    ...
    parser.add_option('-n', '--normalize', dest='normalize',
                      ...
                      help='normalize input signal by removing mean and scale '
                           ' maximum absolute value to 0.98 (or other value '
                           'given with --normalize-max)')
    parser.add_option('', '--normalize-max', dest='normalize_max',
                      type='float', default=0.98,
                      help='Normalize input signal so that maximum absolute '
                      'value reached given value (see -n, --normalize)')
    ...
    parser.add_option('-p', '--output-params', dest='formatparams',
                      ...
                      help='add an output format parameter (can be used'
                           ' multiple times, use -l options to list output '
                           'formats and parameters)')
    parser.add_option('', '--dump-dataflow', dest='dumpDataflow', default='',
                      metavar='FILE', help='output dataflow plan (suitable '
                                           'for process with yaafe-engine)')
    parser.add_option('', '--dump-graph', dest='dumpGraph', default='',
                      metavar='FILE',
                      help='output dataflow in dot format (suitable for '
    ...
    if audiofiles:
        ...
        for pstr in options.formatparams:
            pstrdata = pstr.split('=')
            if len(pstrdata) != 2:
                print ('ERROR: invalid parameter syntax in "%s"'
                       ' (should be "key=value")' % pstr)
                return
